    ...
    IEquatable<BitList>
{
    ...
    /// </summary>
    /// <param name="bit">A bit.</param>
    /// <returns>The <paramref name="bit"/> index.</returns>
    public int IndexOf(bool bit)
    {
    ...
    public int IndexOf(BitList bits)
    {
        for (int i = 0; i < Count; i++)
        {
            if (bits == this[i, bits.Count])
                return i;
        }
        return -1;
    }
    ...
    /// </summary>
    /// <param name="bits">A list of bit.</param>
    public bool Contains(BitList bits)
    {
        return IndexOf(bits) != -1;
    }
    ...
    {
        var newBitList = IsEmpty ? bits : this + bits;
    ...
    {
        var newBitList = IsEmpty ? new BitList(bit) : this + bit;
    ...
    /// </summary>
    /// <param name="bits">A list of bit.</param>
    public void AddFirst(BitList bits)
    {
        Insert(0, bits);
    }
    ...
    /// </summary>
    /// <exception cref="EmptyInstanceException" />
    public bool Remove(BitList bits)
    {
        if (IsEmpty)
    ...
    public void Insert(int index, BitList bits)
    {
        if (IsEmpty)
    ...
    private class BitListEnumerator : IEnumerator<bool>, IDisposable
    {
        public int index;
        public BitList bits;
